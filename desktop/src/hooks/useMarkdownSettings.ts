...
import { updateMarkdownSettings as updateMarkdownSettingsRemote } from '../services/dailyMarkdown';
import { useVaultSync } from './useVaultSync';
import { isLocalMode } from '../services/runtimeMode';
  ...
  const updateSettings = useCallback(
    async (payload: UpdateMarkdownSettingsPayload) => {
      ...
      try {
        ...
        // 已登录态：best-effort 镜像到云端（本地为事实来源，失败不阻塞）
        if (!isLocalMode()) {
          try {
            await updateMarkdownSettingsRemote({
              ...payload,
              saveRootPath: vaultRoot ?? undefined,
            });
          } catch {
            /* 云端镜像失败忽略 */
          }
